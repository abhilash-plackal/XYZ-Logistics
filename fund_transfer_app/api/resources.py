...
class TransactionResource(ModelResource):
    ...
    def obj_create(self, bundle, request=None, **kwargs):
        ...
        if (Group.objects.get(name='root_users').user_set.filter(id=user.id).exists()):
            from_list = Node.objects.filter(name__iexact=bundle.data['from'])
            to_list = Node.objects.filter(name__iexact=bundle.data['to'])
            ...
            if (to_obj.name == from_obj.parent.name) or (from_obj.name == to_obj.parent.name):
                ...
                print('Cant be transfered from `' + from_obj.name + '` to `' + to_obj.name 
